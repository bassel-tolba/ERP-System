<!-- gipcco_project/inventory/services/docs/accounting/period_end.md -->
...
- `run_monthly_depreciation(period: FinancialPeriod)`:
  ...
  - **Workflow:**
    ...
    5. Creates a `DepreciationLog` record for each asset. This action, in turn, triggers a signal that calls `create_je_for_depreciation()` (from `asset_transactions.py`) to post the individual journal entry.
