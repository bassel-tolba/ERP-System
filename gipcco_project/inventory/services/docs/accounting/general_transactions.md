...
- `create_je_for_internal_consumption(consumption: InventoryConsumption)`:
  - **Description:** Creates a journal entry for items consumed internally rather than sold.
  - **Logic:**
  ...
  - **Calls:** `_check_period_is_open()`, `_get_product_inventory_account()`, `_get_product_expense_account()` from `_helpers.py`.

- `create_je_for_bank_transfer(transfer: BankTransfer)`:
  - **Description:** Creates a journal entry for moving funds between two internal bank accounts.
...
- `create_je_for_expense_log(expense_log: ExpenseLog)`:
  - **Description:** Creates the initial journal entry for a direct expense, accruing the liability.
...
- `create_transaction_for_direct_payment_expense(request: ExpenseRequest)`:
  - **Description:** Handles an expense that is paid directly via a bank transfer, bypassing the usual accrual process.
...
- `create_je_for_opening_balance(ob_entry: OpeningBalanceEntry)`:
  - **Description:** Creates the master, multi-line journal entry from a prepared `OpeningBalanceEntry` record. This is a critical step in data migration.
